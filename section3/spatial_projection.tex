...
\KTC{I removed the remark here, as it should be obvious that stationarity
conditions are just necessary optimality conditions.} \KTC{Also, this
derivation is really, really cool.}
%\begin{remark}
...
problem from optimal control. \KTC{I didn't understand the previous sentence.
Can you clarify how this is exposed?} To this end, consider the Galerkin ROM over the $n$th window
...
and pose the problem of finding a a controller that minimizes the residual
over the time window and forces the dynamics as \KTC{EP: check to see if you
like/dislike previous sentence}
...
for truncated states.  \KTC{EP verify: I changed ``truncated dynamics'' to
``truncated states''.}
...
\end{split}
\end{equation}
\KTC{EP verify that the Lagrangian above lives in $\RRplus{}$. This is what I
just changed.} where we have used $ \genstateDotArg{n}{t} = [\massArg{n}]^{-1}\basisspaceTArg{n}
...
modifies the Galerkin ROM to minimize the residual over the time window. In
addition to commonalities with traditional optimal-control problems, this
formulation displays commonalities with subgrid-scale modeling
techniques, wherein an additional term is added to the reduced-order model to
account for the impact of the truncated modes. \KTC{This was just said above;
do we need to say it again?}. The \methodAcronym\ method can thus be
