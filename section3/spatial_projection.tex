...
The \spatialAcronym\ trial space over the $n$th time window is given as, 
\begin{equation}\label{eq:sttrialspace}
 \approxstateArgnt{n} \in \stspaceSArg{n} \defeq \trialspace \otimes \timeSpaceArg{n},
\end{equation}
where the spatial trial space, $\trialspace$, is as defined in~\eqref{eq:spatial_subspace2}-\eqref{eq:spatial_subspace3}.
% and  
...
where the spatial trial space is equivalent over each time window. Over the $n$th time window, the state is approximated within the trial space as, 
...
%\begin{split}
%      & \underset{\genstatey \in \RR{\romdim}}{\text{minimize}}\; \mathcal{J}^n(\basisspace \genstatey + \stateIntercept), \\ 
