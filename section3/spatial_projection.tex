...
$t\in[\timeStartArg{n},\timeEndArg{n}]$ \KTC{I changed this because we are
only approximating the solution in the $n$th time window.}
...
$ \KTC{Have we defined $\RRStar{m}{n}$?}, the reference state $\stateInterceptArg{n} \in \RR{\fomdim}$, $n=1,\ldots,\nslabs$ provides the affine transformation, 
and  
...
\KTC{This optimization is defined incorrectly. It shoudl just be $\hat y\in
ST_S^n$ under minimize and $J^n$ should take only $\hat y$ in the argument.}
...
\textit{function} $\genstate^n\in\stspaceSArg{n}$ that minimizes the \textit{functional}
...
We also define the quantities\footnote{We use Jacobian layout for the
scalar-by-vector gradients \KTC{Not sure whta preceding statement means. I
would remove this footnote as the dimensions are clear from what you write
below.}, and, as a result, the transpose operator is needed for, e.g., $\dIdVArg{n}$ is a column vector functional.}
\begin{equation}
\begin{split}
