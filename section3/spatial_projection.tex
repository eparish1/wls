...
\begin{split}
	...
	\begin{cases}
		[\basisspaceArg{n}]^T[\genstateArg{n-1}{\timeStartArg{n}}-\stateInterceptArg{n}] & n = 2,\ldots,\nslabs \\
...
We emphasize that problem~\eqref{eq:obj_gen_slab} 
constitutes 
	aims to compute the
function $\genstate^n$ that minimizes the functional
	$\objectiveArg{n}$. \KTC{I removed the `infinite dimensional' description
	here to avoid confusion and keep it precise. Is that okay? It would be
	finite dimensional if we introduced a finite-dimensional class of
	time-dependent functions instead of $\timeSpaceArg{n}$.}
	%corresponds to
...
%& \dIdY(\genstateArg{n}{t},\genstateDotArg{n}{t},t) - \dIdVDot(\genstateArg{n}{t},\genstateDotArg{n}{t} ,t )  = \bz, \qquad t \in [\timeStartArg{n},\timeEndArg{n}],\\ 
%& \frac{\partial
%\minintegrandArg{n}}{\partial
%\genstateyDiscrete}(\genstateArg{n}{t},\genstateDotArg{n}{t},t) - \frac{d}{dt} \bigg(
% \frac{\partial \minintegrandArg{n}}{\partial \genstateyDiscreteDotArgnt{}} 
%(\genstateArg{n}{t},\genstateDotArg{n}{t} ,t) \bigg) = \bz, \qquad t \in [\timeStartArg{n},\timeEndArg{n}],\\ 
...
Also make sure that $\romdim$ is replaced with $\romdimArg{n}$ everywhere.} We introduce now the controller $\controllerArgnt{n} :  [\timeStartArg{n},\timeEndArg{n}] \rightarrow
\RR{\romdim}$,
