...
\end{comment}
\KTC{Next sentence, use parallel language to the last section. It's subtly
different but I think written better there.} Substituting the approximation~\eqref{eq:stapprox} into the minimization problem~\eqref{eq:tclsrm}, \methodAcronym\ with 
...
\end{split}
\end{align}
\KTC{In prior equation, remove projection and align properly with ST-LSPG
stuff before.}
...
respect to vector.  As such, the first-order optimality conditions can be
	...
	to $\stgenstatey$ and setting equal to zero yields \KTC{Make sure that xref
	is replaced with the last solution at the previous time step}
