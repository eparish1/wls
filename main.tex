...
of existing model reduction approaches, as particular instances of
  the methodology recover the Galerkin,
	least-squares Petrov--Galerkin (LSPG), and space–time LSPG approaches. In
	addition, the approach
	addresses key deficiencies in existing model-reduction
	techniques, e.g., the dependence of the (space--time) LSPG approach on the
	time discretization and the exponential growth in time exhibited by \textit{a posteriori}
	error bounds for both the Galerkin and LSPG approaches.  We consider two types of
	space--time trial
	subspaces within the proposed approach: one that associates with spatial projection only, and one that 
  associates with space--time projection. For each type of trial
	...
	projection only demonstrate that the \methodAcronym\
	approach can yield more accurate solutions with lower space--time residuals than
	the Galerkin and LSPG approaches. 
...
formulation and examined its connections with the Galerkin approach. Critically,
this work demonstrated that (1) under certain conditions, LSPG can be cast as a 
Petrov--Galerkin approach applied to the time-continuous
full-order-model residual, and (2) the LSPG and Galerkin approaches are equivalent
in the limit as the time step goes to zero (i.e., the Galerkin approach 
...
As LSPG and Galerkin are equivalent in the
...
Galerkin, LSPG, and space--time LSPG correspond to specific
...
\EP{Updated this, I don't show what you said}%As a result,
...
space--time LSPG approaches. Section~\ref{sec:tclspg} outlines the proposed \methodAcronym\
approach. Section~\ref{sec:numerical_techniques}
...
\begin{enumerate} 
	\item \textit{Subspaces that reduce only the spatial dimension of the full-order
		model (\spatialAcronym)}. These trial spaces are characterized by a \textit{spatial projection} process and are employed in classic model reduction approaches, e.g., Galerkin and LSPG. %These 
...
\subsubsection{Galerkin approach}
...
%\end{equation}
\subsubsection{The Least-squares Petrov--Galerkin approach}
...
$\stbasis:\timeDummy\mapsto\stbasis(\timeDummy)$ and $\stbasis(0) =
\boldsymbol 0$
denotes the space--time trial basis.\footnote{For simplicity, we consider the case where the trial space is homogeneous on the boundary.} 
...
\end{equation}
where $\lspgWeightingSTArg{\cdot} \in \RR{\stdim \times N N_t}$, with $\stdim
...
a dense systems that do not expose any natural mechanism for exploiting
...
O$\Delta$E residual over all time instance.
...
complex dependence of LSPG projection on the time discretization,
exponential time growth of the error bounds for Galerkin and LSPG projection,
the cubic dependence of the computational cost of ST-LSPG on the number of
degrees of freedom, and the lack of ability for ST-LSPG to perform predictions
in time. We now describe the proposed \methodNameLower\ framework for this
purpose. 
%\end{comment}
%\input{formulation_3}
...
We define a new function that is equivalent to~\eqref{eq:el_gen} evaluated at $\statezBar(t,\cdot)$,
$$
...
%$$
The objective is to now find $\overline{\statez}$ that makes $\mathcal{J}_{\delta}$ stationary. This can be done by differentiating with respect to $\delta$, and setting the result to zero,% Noting that the extermum of Eq.~\ref{eq:el_gen} occurs when $\state = \state^*$, and thus when $\epsilon = 0$, we have,
$$
