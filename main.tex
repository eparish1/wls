...
Residual minimization methods in model-order reduction comprise 
computing a solution within a low-dimensional trial space that minimizes some residual norm of the system of interest. Researchers have developed such residual-minimizing
...
The LSPG approach has several shortcomings. First, the performance of LSPG depends on the
time discretization. Changing the time-step size modifies both the
...
Second, as the LSPG approach sequentially performs residual minimization over
time-steps, is not equipped with any notion of optimality over the entire 
time domain of interest. As a result, LSPG is not equipped with \textit{a priori}
guarantees of stability or convergence, even for linear time-invariant
systems~\cite{bui_thesis}.

Researchers have pursued the development of space--time residual minimization ROMs~\cite{choi_stlspg,constantine_strom,URBAN2012203,Yano2014ASC,bui_thesis} to address this last issue. In existing space--time methods, the spatial \textit{and} temporal dependence of the state is represented in a low-dimensional space--time trial space. Space--time residual minimization approaches then comprise computing a space--time solution within this low-dimensional space--time trial space that minimizes the space--time norm of the full-order model. Space--time methods are thus equipped with a notion of optimality over the entire time domain of interest and, as a result, contain more favorable error bounds than the Galerkin and LSPG approaches.  Several drawbacks of existing space--time ROMs are that they (1) require minimizing the entire space--time residual (which can be a computationally intensive process) and (2) require the specification of a finite-dimensional temporal basis (computed, e.g., via POD applied to space--time snapshots). This latter issue is extremely relevant to the present work.  
...
approaches. 
>>>>>>> e51b3a7cf77e04030f1e7c1d96083a13d48c32af
...
space--time LSPG ROMs; Section~\ref{sec:tclspg} outlines the proposed \methodAcronym\
formulation; Section~\ref{sec:numerical_techniques}
...
We denote vector-valued functions with stylized bold text (i.e., $\boldsymbol
x$), vectors with standard bold text (i.e., $\mathbf{x}$), 
matrices with capital bold letters (i.e., $\mathbf{X} \equiv \begin{bmatrix}
\mathbf{x}_1 & \cdots & \mathbf{x}_r\end{bmatrix}$), and spaces with
calligraphic fonts (i.e., $\mathcal{X}$).
...
\begin{equation}\label{eq:FOM}
 \frac{d\stateFOM}{dt} = \velocity(\stateFOM,t), \qquad \stateFOM(0) = \stateFOMIC,
\end{equation}
where $t \in [0,T]$ denotes time with 
$T \in \mathbb{R}^+$ denoting the final time, $\stateFOM \in \RR{\fomdim}
	...
	problem~\eqref{eq:FOM}, $\stateFOMIC \in \mathbb{R}^{\fomdim}$ denotes the initial conditions,
...
\begin{enumerate} 
	\item \textit{\spatialAcronym\ trial subspaces}. These 
	...
	\item \textit{\spaceTimeAcronym\ trial subspaces}. These 
