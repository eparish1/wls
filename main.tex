...
The authors thank Yukiko Shimizu and Patrick Blonigan for numerous conversations from which this work benefited. 
...
\label{alg:pod}
\textbf{Input:} Number of time-steps between snapshots $N_{\text{skip}}$; intercept $\stateInterceptArg{}$; basis dimension $K$; weighting matrix, $\mathbf{P}$ \; 
%\newline
\textbf{Output:} POD Basis $\basisspace \in \{ \mathbf{X} \in \RR{\fomdim \times \romdim} | \mathbf{X}^T\mathbf{P} \mathbf{X} = \mathbf{I} \} $ \;
%\newline
\textbf{Steps:}
\begin{enumerate}
...
$$\mathbf{S}(N_{\text{skip}}) \defeq \begin{bmatrix} \stateFOMDiscreteArg{0} - \stateInterceptArg{n} & \stateFOMDiscreteArg{N_{\text{skip}}} - \stateInterceptArg{n} & \cdots & \stateFOMDiscreteArg{\text{floor}(N_t/N_{\text{skip}})N_{\text{skip}}} - \stateInterceptArg{n} \end{bmatrix}$$
    \item Compute the thin singular value decomposition, $$ \sqrt{\mathbf{P}}^T\mathbf{S} (N_{\text{skip}}) = \mathbf{U \Sigma Z}^T,$$
    where $\mathbf{U} \equiv \begin{bmatrix} \mathbf{u}_1 & \cdots & \mathbf{u}_{\text{floor}(N_t/N_{\text{skip}})}\end{bmatrix}$ and $\sqrt{\mathbf{P}}$ is the Cholesky decomposition of $\mathbf{P}$, i.e., $\sqrt{\mathbf{P}} \sqrt{\mathbf{P}}^T = \mathbf{P}$
    \item Truncate left singular vectors and to form the basis, $\basisspace \equiv \begin{bmatrix} [\sqrt{\mathbf{P}}^T]^{-1}  \mathbf{u}_1 & \cdots & [\sqrt{\mathbf{P}}^T]^{-1} \mathbf{u}_K \end{bmatrix}$
\end{enumerate}


\end{algorithm}

%In the final numerical experiment, time-local trial bases are constructed for a set of varying input parameters. Algorithm~\ref{alg:pod_tlp} describes the construction of the POD bases for this case.
%\begin{algorithm}
%\caption{Algorithm for generating time-local POD basis over multiple parameter instances}
%\label{alg:pod_tlp}
%\textbf{Input:} Number of time-steps between snapshots $N_{\text{skip}}$; starting time-instance, $N_s$; number of instances $N_{i}$; intercept $\stateInterceptArg{}$; parameter instances $\param_{1} , \ldots, \param_{n_{\text{train}}}$; basis dimension $K$ \\
%%\newline
%\textbf{Output:} POD Basis $\basisspace \in \mathbb{V}_{K}(\RR{N})$ \;
%%\newline
%\textbf{Steps:}
%\begin{enumerate}
%    \item Solve FOM O$\Delta$E for $\param_1, \ldots, \param_{n_{\text{train}}}$ and collect solutions into snapshot matrix
%\small{$$\mathbf{S}(N_{\text{skip}}) \defeq \begin{bmatrix} \stateFOMDiscreteParamArg{N_s}{\param_1} - \stateInterceptArg{n} & \cdots & \stateFOMDiscreteParamArg{N_s + N_i N_{\text{skip}}}{\param_1}- \stateInterceptArg{n} & \cdots & \stateFOMDiscreteParamArg{N_s + N_i N_{\text{skip}}}{\param_{n_{\text{train}}}}- \stateInterceptArg{n} \end{bmatrix}$$}
...
$$\mathbf{F}(N_{\text{skip}}) \defeq \begin{bmatrix} \stateFOMDiscreteArg{0} & \stateFOMDiscreteArg{N_{\text{skip}}}  & \cdots & \stateFOMDiscreteArg{\text{floor}(N_t/N_{\text{skip}})N_{\text{skip}}} \end{bmatrix}$$
    ...
    \item Augment the sampling point matrix, $\stweightingMatOneArg{}$, with additional columns such that all unknowns are computed at the mesh cells selected by Step 3. For the second numerical experiment, these additional unknowns correspond to each conserved variable and degree of freedom in the selected cells. 
\end{enumerate}
