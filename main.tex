...
\spatialAcronym\ trial subspaces approximate the FOM ODE solution
	$\stateFOM(t)$, $t\in[0,T]$ with an approximation $\approxstate(t)$, $t\in[0,T]$ that resides in an
...
\end{split}
\end{equation}
Here, $\trialspace\subseteq\RR{\fomdim}$ with $\dim(\trialspace) = K$ denotes
the spatial trial subspace spanned by an orthogonal basis such that
...
%	domain
%	$[0,T]$, 
We have also introduced $\onesFunction\in\timeSpace$ as the function $\onesFunction:\timeDummy\mapsto 1$
and
	$\stateIntercept \in \mathbb{R}^{\fomdim}$ as the reference state, which
...
\end{align*}
with $\Range{\stbasis} + \stateInterceptArg{} \equiv \stspaceST$ and where $\stdim$ is the dimension of the space--time generalized coordinates.
...
\stbasisDotArg{}{t} \stgenstate =  \velocity(\stbasisArg{}{t} \stgenstate + \stateIntercept,t) , \qquad \genstate(0) =\mathbb{P}(\stateFOM)(0), \qquad t \in [0,T],
\end{equation}
