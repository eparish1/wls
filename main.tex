...
\begin{equation}\label{eq:FOM}
 \frac{d\stateFOM}{dt} = \velocity(\stateFOM,t), \qquad \stateFOM(0) =
...
$T \in \mathbb{R}^+$ denotes the final time, $\stateFOM \in \RR{\fomdim}
	...
	problem~\eqref{eq:FOM}, $\stateFOMIC \in \mathbb{R}^{\fomdim}$ denotes the initial condition,
...
& \trialspace \defeq \text{Range}(\basismat),  \label{eq:spatial_subspace2}  \\
...
where 
	...
	domain
	$[0,T]$, $\onesFunction\in\timeSpace$ is defined as $\onesFunction:[0,T]\rightarrow\{1\}$,
...
coordinates. Substituting the approximation~\eqref{eq:affine_trialspace} in
Eq.~\eqref{eq:FOM} and performing orthogonal
...
\basisspace \frac{d\genstate }{dt} = \velocity(\basisspace
\genstate + \stateIntercept,t ), \qquad \genstate(0) = \genstateIC, \qquad t \in [0,T].
\end{equation}
...
Eq.~\eqref{eq:g_truncation} by enforcing orthogonality of the associated
residual to the trial subspace in the (semi-)inner product induced by the 
positive (semi-)definite matrix $\stweightingMatArg{}\equiv \stweightingMatOneArg{ }^T
\stweightingMatOneArg{ }$ (commonly set to
$\stweightingMatArg{}=\mathbf{I}$), i.e.,
\begin{equation*}\label{eq:g_truncation2}
\frac{d\genstateGalerkin}{dt} = \massArgnt{n}^{-1} \basisspace^T \stweightingMatArg{n} \velocity(\basisspace
\genstateGalerkin + \stateIntercept,t), \qquad \genstateGalerkin(0) = \genstateIC, \qquad t \in [0,T],
\end{equation*}
where $\massArgnt{ } \equiv \basisspace^T \stweightingMatArg{ } \basisspace$
denotes a mass matrix.  Note that the Galerkin ODE~\eqref{eq:g_truncation2} is
...
method. In particular, the Galerkin ODE~\eqref{eq:g_truncation2} is equivalent
to
\begin{equation}\label{eq:GalOptimal}
\frac{d\genstateGalerkin}{dt} = \underset{ \genstateyDiscrete \in \RR{\romdim}
	}{\text{arg\,min}} \| \basisspace \hat{\stateyDiscrete} -
	\velocity(\basisspace \genstateGalerkin + \stateIntercept,t)
...
%\end{equation*}
Thus, velocity $\frac{d\genstateGalerkin}{dt}(t)$ computed by Galerkin projection minimizes the
time-instantaneous FOM ODE residual evaluated at the state $\basisspace
\genstateGalerkin(t) + \stateIntercept$ and time $t$.
...
Despite its time-instantaneous optimality property expressed in
Eq.~\eqref{eq:GalOptimal}, Galerkin projection can yield unstable and
inaccurate trajectories, particularly if the velocity is not self-adjoint or
is nonlinear. 
...
is an alternative approach that exhibits several advantages over Galerkin
projection.
Rather than minimize the (time-continuous) FOM ODE residual as in Galerkin
projection, LSPG minimizes
the residual arising at a given time step after applying time discretization to the FOM ODE; we
refer to the residual arising at the fully discrete level as the FOM O$\Delta$E residual.  
We now describe LSPG projection when a linear multistep method is employed for
time discretization; Ref.~\cite{carlberg_lspg_v_galerkin} derives LSPG projection for Runge--Kutta
schemes as well. 
\KTC{HERE}
Defining a discretization
of the continuous time domain $[0,T]$ into $N_t$ time instances $t^n = n\Delta
t, n=1,2,\ldots,N_t$ with a constant time step $\Delta t = T/N_t$, temporal
discretization via a linear multistep method yields the FOM O$\Delta$E,
