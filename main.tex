...
This work proposes a \methodNameLower\ (\methodAcronym) model-reduction
	...
	time-continuous full-order-model residual over a low-dimensional trial
	subspace across a sequence of time windows and addresses key deficiencies in
	existing model-reduction approaches, e.g., the time-step and time-scheme
	dependence of least-squares Petrov--Galerkin (LSPG) projection, \textit{a
	posteriori} error bounds that grow exponentially in time for Galerkin and
	LSPG projection, and the offline cost of space--time LSPG projection.
	\KTC{How do we address the offline cost of ST-LSPG?}
We consider two types of trial subspaces
	within the proposed framework:
	one that reduces only the spatial dimension of the full-order model, and one that
	reduces both the spatial and temporal dimensions of the full-order model.
	For each type of trial subspaces, we consider two different solution
	techniques: direct (i.e., discretize-then-optimize) and indirect (i.e.,
	optimize-then-discretize). Critically, we show that various instances of
	this framework yield existing model-reduction approaches, namely Galerkin,
	LSPG, and space–time LSPG projection. Numerical experiments on the
	compressible Euler and Navier--Stokes equations demonstrate that, at an
	increased computational cost, the \methodAcronym\ formulation can yield more
	accurate and physically relevant solutions than Galerkin and
	LSPG projection.  \KTC{So which is better? Should we emphasize that it is
	more general?}
...
%\KTC{I reworked this. Methods like balanced truncation or IRKA do \textit{not}
%use snapshot data!}
...
that such properties are difficult to quantitatively assess. As a result, it
is difficult to develop reduced-order models that preserve important
dynamical-system properties, which often results in inaccurate or unstable
methods.  To address this, researchers have pursued several directions that
...
projection that equips the ROM solution with a notion of optimality~\cite{l1,carlberg_lspg,carlberg_gnat,legresley_1,legresley_2,legresley_3,bui_resmin_steady,bui_unsteady,rovas_thesis,carlberg_thesis,bui_thesis};
space--time
...
Researchers also developed residual-minimization methods for model reduction.
Similar to LSFEM, residual minimization in this context associates with
computing the solution within a low-dimensional trial space that minimizes the
$\ell^2$-norm of the full-order-model residual. \KTC{haven't defined
`full-order model'} Researchers have developed such residual-minimizing
model-reduction methods for both static systems (i.e., systems without
time-dependence)
...
full-order-model residual, and (2) LSPG and Galekrin projection are equivalent
...
than time instantanously), LSPG projection computes solutions that are more
accurate over a larger part the trajectory compared with Galerkin projection.


Residual minimization methods were extended-to and developed-for reduced-order
models by a variety of researchers. Similar to LSFEM, residual minimization
approaches for reduced-order models comprise finding the solution within a
low-dimensional trial subspace that minimizes the residual norm of the
full-order model. Residual minimization approaches were first applied to
static (i.e., no time-dependence) reduced-order models
in~\cite{legresley_1,legresley_2,legresley_3,bui_resmin_steady,rovas_thesis,carlberg_thesis,bui_thesis}
and then extended to dynamical systems
in~\cite{bui_thesis,bui_unsteady,carlberg_thesis,carlberg_gnat,carlberg_lspg,carlberg_lspg_v_galerkin}.
Specifically,
Refs.~\cite{bui_thesis,bui_unsteady,carlberg_thesis,carlberg_gnat,carlberg_lspg}
formulated the residual minimization problem for dynamical systems by
sequentially minimizing the \textit{fully discrete} residual (i.e., the
residual after temporal discretization) at each time-step instance. This
formulation is most commonly known as the \textit{Least--Squares
Petrov--Galerkin} (LSPG) method. Ref.~\cite{carlberg_lspg_v_galerkin} provided
numerous analyses on this formulation and examined connections with the
standard Galerkin method and Petrov--Galerkin methods. Critically, it was
shown that (1.) for various time-stepping methods LSPG has an equivalent
continuous form that corresponds to a Petrov--Galerkin projection and (2.) the
standard Galerkin method minimizes the time-instantaneous residual of the
full-order model. Numerous numerical experiments have demonstrated that LSPG
yields more stable (although not always more accurate) solutions than the
Galerkin
method~\cite{bui_thesis,carlberg_lspg_v_galerkin,carlberg_gnat,carlberg_thesis,parish_apg}.
The common physical explanation for this improved performance is that, by
minimizing the fully discrete residual, LSPG minimizes the residual over a
\textit{finite-sized time-window}, rather than instantaneously as is done in
the Galerkin method.
 
\KTC{Let's discuss. What specifically do we want to point out about
deficiencies in LSPG in this section? Should we also look in ST-LSPG? It seems you're trying
to say that we don't like how the time step size \textit{and} time
discretization method dictate so much about the method (I agree with this). If
so, we should structure this to include other discretize-then-optimize methods
I think. Or we could just focus on the time-step aspect (i.e., not the time
discretization method), in which case we should focus on LSPG.}
\KTC{HERE}

The LSPG approach sequentially performs residual minimization over a single
time-step, and as such is not associated with residual minimization over the
entire time interval. \KTC{This is what ST-LSPG does. Why do we ignore it? the
...
propose the \methodNameLower\ (\methodAcronym) method for model reduction,
which minimizes the FOM ODE residual over a sequence of arbitrarily defined
time windows. The key novelties of the work are twofold.  First, the method
...
residual minimization problem, thus addressing a key deficiency of LSPG
\KTC{This is also true for ST-LSPG and maybe other discretization-based
methods? Have to check}. Time-continuous residual minimization also exposes
two different solution methods,  namely a \textit{discretize-then-optimize}
(i.e., direct) method and an \textit{optimize-then-discretize} (i.e.,
indirect) method. The second novelty is that the method sequentially minimizes
the residual over arbitrarily defined \textit{time windows} rather than
sequentially minimizing the residual over time steps or over the entire time
domain. This equips the method with additionaly flexibility that enables it to
explore more fine-grained tradeoffs between computational cost and error. We
demonstrate that this method can be viewed as a generalization of existing
methods, because particular instances of this formulation are equivalent to
Galerkin, LSPG, and space--time LSPG projection. Figure~\ref{fig:flowchart}
depicts how the proposed \methodAcronym\ method is related to existing
approaches. 
...
(1.) in addition to trial subspaces with space--time dimension reduction, we
consider trial subspaces with spatial dimension reduction only (i.e., a classic
...
domain within a given space--time trial subspace. 
%These space--time trial spaces are constructed through higher-order singular value decompositions (SVDs), such as the truncated high-order SVD. 
...
consider trial subspaces with both space--time dimension reduction and spatial
...
fully-discrete residual over the entire time domain. 
...
\item The development of the \methodNameLower\ (\methodAcronym) approach for dynamical systems. The approach sequentially minimizes the time-continuous full-order model residual over time windows.
\item The derivation of the stationary conditions for \methodAcronymROMs\ leveraging spatial and space--time dimension reduction trial subspaces. The case of spatial dimension reduction trial subspaces is of particular interest: the stationary conditions are derived via the Euler--Lagrange equations and comprise a coupled 
...
Projection-based ROMs generate approximate solutions to the FOM ODE~\eqref{eq:FOM} by approximating the state in a low-dimensional trial subspace. For dynamical 
systems, two distinct types of trial subspaces are used: space--time trial subspaces with spatial dimension reduction (\spatialAcronym) and space--time trial subspaces with space--time dimension reduction (\spaceTimeAcronym). \spatialAcronym\ trial subspaces are most commonly employed and are used in, e.g., the Galerkin and Least-Squares Petrov--Galerkin (LSPG) methods. \spaceTimeAcronym\ trial subspaces are typically 
employed in space--time ROMs. The following subsections outline these two trial subspaces, along with the Galerkin, LSPG, and 
space--time LSPG method. 

\subsection{\spatialAcronym\ Trial Subspaces}
\spatialAcronym\ trial subspaces approximate the FOM ODE solution, $\stateFOM$, with an approximate solution, $\approxstate$, that lives in a $K$-dimensional affine trial subspace, 
...
\end{align}
where we refer to $\basismat \in \RRStar{\romdim}{\fomdim}$, with $\RRStar{\romdim}{\fomdim}$ denoting the set  $\{ \mathbf{X} \in \RR{\fomdim \times \romdim} \big| \mathbf{X}^T \mathbf{X} = \mathbf{I} \}$, as the spatial trial basis matrix, $\trialspace$ as the spatial trial subspace, and $\stateIntercept \in \mathbb{R}^{\fomdim}$ as the reference state. 
%where $\basisvec_i \in \mathbb{R}^N$ are orthonormal basis vectors and $\stateIntercept \in \mathbb{R}^{\fomdim}$ the ``intercept''.
%We refer to $\basismat \in \RRStar{\fomdim \times \romdim}$, with $\RRStar{p \times q}$ denoting the set of orthogonal $q$ frames in $p$, as the spatial trial basis, and $\trialspace$ as the spatial trial space. 
...
ROMs leveraging \spatialAcronym\ trial subspaces approximate the state as,
...
The Galerkin method is (traditionally) obtained by the following steps: (1.) The approximation~\eqref{eq:affine_trialspace} is substituted into~\eqref{eq:FOM}, and (2.) the resulting residual is enforced to be $\stweightingMatArg{}$-orthogonal to the trial subspace, with $\stweightingMatArg{ } \equiv \stweightingMatOneArg{ }^T \stweightingMatOneArg{ }$ being a positive (semi-)definite weighting matrix (commonly taken to be $\mathbf{I}$). This projection process results in what is known as the Galerkin method,
...
The FOM O$\Delta$E solution will exactly satisfy~\eqref{eq:lms}; the ROM solution will not. The LSPG method minimizes the residual of the FOM O$\Delta$E~\eqref{eq:lms} within the trial subspace over each time step. %We define the discrete residual for the implicit Euler method at the $n$th time-instance as,
%\begin{align*}
%\resid_{\text{IE}} & \vcentcolon (\stateyDiscrete;\stateFOMDiscreteArg{n-1}) \mapsto \frac{\stateyDiscrete - \stateFOMDiscreteArg{n-1}  }{\Delta t} -  \velocity(\stateyDiscrete), \\
%& \vcentcolon \RR{\fomdim} \times \RR{\fomdim} \rightarrow \RR{\fomdim}. 
%\end{align*}
...
\stbasisArgnt{} &: t \mapsto \stbasis(t) \\
 &: [0,T] \rightarrow  \RR{N \times \stdim} ,%\spaceTrialSpace , \\
%&  \in \RR{N \times \stdim} \otimes \timeTrialSpaceArg{},  
\end{align*}
...
%with $\mathbb{V}_{\stdim}( \RR{N} )$ denoting the set of orthogonal $\stdim$ frames in $\RR{N}$ (e.g., an $N \times \stdim$ matrix with orthogonal column vectors).
...
\end{equation}
%where $\stbasisArgnt{} : \RR{\stdim} \rightarrow \spaceTrialSpace \otimes \timeTrialSpaceArg{}$ with $ \stbasisArgnt{n} \in \RR{N \times \stdim} \otimes \timeTrialSpaceArg{n}$ is the space--time trial basis, 
...
\end{equation}
where $\integrand: \RR{M} \times \RR{M} \times[a,b] \rightarrow  \RR{}$ (for arbitrary $M$) with $\integrand: (\stateyDiscrete,\stateyDotDiscrete,t) \mapsto \integrand(\stateyDiscrete,\stateyDotDiscrete,t)$.%, $\statez: [a,b] \rightarrow \RR{M}$ and $\statezDot: [a,b] \rightarrow \RR{M}$.
 We now introduce the function $\statez: [a,b] \rightarrow \RR{M}$ that is implicitly defined as an extremum of~\eqref{eq:el_gen} subject to the boundary condition $\statez(a) = \statez_a$. %To this end, we start by supposing that $\statez$ is a stationary point of $\mathcal{J}$ that satisfies the boundary conditions. 
