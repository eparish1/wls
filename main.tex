...
of existing model reduction approaches, as particular instances of
  the methodology recover the Galerkin,
	...
	techniques, e.g., the dependence of (space--time) LSPG on the
	time discretization and the exponential growth in time exhibited by \textit{a posteriori}
	error bounds for both Galerkin and LSPG.  We consider two types of
	space--time trial
...
In particular, because LSPG and Galerkin are equivalent in the
limit as the time step goes to zero, the accuracy of LSPG approaches the (poor)
...
\EP{This has nothing to do the basis, which was the point I was trying to get across}
...
full-order model (as employed by space--time methods).
...
aim to overcome the latter two shortcomings of LSPG, which are
shared by all model-reduction methods based on spatial projection followed by
time integration. \EP{This is misleading, going to delete} Because these methods employ \spaceTimeAcronym\ trial
...
While space--time Galerkin projection \cite{benner_st} is another alternative, it does not
...
\end{align*}
ST-LSPG projection computes the space--time generalized coordinates that
...
degrees of freedom $\stdim$, because it
leads to dense systems that do not expose any natural mechanism for exploiting
