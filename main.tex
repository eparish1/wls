...
\begin{enumerate} 
	\item \textit{Subspaces that reduce only the spatial dimension of the full-order
		model (\spatialAcronym)}. These trial subspaces are characterized by a \textit{spatial projection} and are employed in classic model reduction approaches, e.g., Galerkin and LSPG. %These 
...
These trial subspaces are characterized by a \textit{space--time} projection and are employed in space--time 
...
it further.
\subsubsection{The space--time LSPG approach}  
