...
This work proposes a \methodNameLower\ (\methodAcronym) model-reduction
	...
	domain of interest.  We consider two types of space--time trial subspaces
	...
	a list of interesting factoids.} The stationary conditions for the resulting minimization problems are
	derived.  Of particular interest, the stationary conditions for trial spaces
	with spatial dimension reduction are obtained via the Euler--Lagrange
	equations and comprise a coupled two-point Hamiltonian boundary value
	problem containing a forward and backward system. 
%The forward system, which is forced by an auxiliary costate, evolves the generalized coordinates of the reduced-order model. The backwards system, which is forced by the FOM residual evaluated about the ROM state, governs the dynamics of the costate.
...
We consider two methods to solve the residual-minimization problems: direct
	...
	optimize-then-discretize).  Critically, we show that the proposed method
...
discretizing a partial-differential-equations problem) and/or when the number
...
structured dynamical systems) is quite mature \cite{wilcox_benner_rev}
\KTC{Also cite classical refs on balanced truncation, Gugercin's IRKA,. Note
that I moved \cite{wilcox_benner_rev} here because that review mainly deals
with linear systems}, as system-theoretic properties (e.g., controllability,
...
that such properties are difficult to assess for such systems. As a result, it
is difficult to develop projection methods that preserve important
dynamical-system properties in the reduced-order model, which can lead to
models that are inaccurate or unstable.  To address this, researchers have
...
``energy-based'' inner
products that ensure non-increasing entropy in the ROM solution~\cite{rowley_pod_energyproj,Kalashnikova_sand2014}; symmetry
transformations that \KTC{what?} ~\cite{sirovich_symmetry_trans},
basis-adaptation methods that improve the ROM's accuracy \textit{a posteriori}~\cite{carlberg_hadaptation,adeim_peherstorfer}, subspace
rotations that \KTC{what?} ~\cite{basis_rotation}, structure-preserving
methods that enforce conservation \cite{carlberg_conservative_rom} or
(port-)Hamiltonian/Lagrangian structure
\cite{carlberg2012spd,structurePreserveBeattie,chaturantabut2016structure,farhat2014dimensional}
\KTC{Can use more here. See my presentation} in the ROM; and subgrid-scale
