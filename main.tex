...
This work proposes a \methodNameLower\ (\methodAcronym) model-reduction
	...
	subspace across time windows. The approach comprises a generalization 
of existing model reduction approaches; we show that various instances of
  the method recover the Galerkin,
  least-squares Petrov--Galerkin (LSPG), and space–time LSPG approaches. The approach further addresses key deficiencies in
	existing model-reduction techniques, e.g., the dependence of (space--time) LSPG 
on the time-discretization and the exponentially growing \textit{a
	posteriori} error bounds present in Galerkin and
	LSPG.
We consider two types of trial subspaces
	...
	%LSPG, and space–time LSPG projection. 
  Numerical experiments are presented for trial subspaces with spatial dimension reduction, in where it is demonstrated that the \methodAcronym\ formulation can yield more
...
Lyapunov equations, etc). Second, these methods execute an inexpensive
...
computing a solution within a low-dimensional trial subspace that minimizes some residual norm of the system of interest. Researchers have developed such residual-minimizing
...
The LSPG approach, however, has several shortcomings. First, the performance of LSPG depends on the
...
Researchers have pursued the development of residual-minimizing space--time projection~\cite{choi_stlspg,constantine_strom,URBAN2012203,Yano2014ASC} to address this last issue of global optimality. 
%In existing space--time methods, the spatial \textit{and} temporal dependence of the state is represented in a low-dimensional space--time trial space. 
Space--time residual minimization approaches comprise computing a space--time solution within a low-dimensional space--time trial space that minimizes the space--time norm of the full-order model. Space--time methods are thus equipped with a notion of optimality over the entire time domain of interest and, as a result, contain more favorable error bounds than the Galerkin and LSPG approaches.  Several drawbacks of existing space--time ROMs are that they (1) require minimizing the entire space--time residual (which can be a computationally intensive process) and (2) require the specification of temporal basis (computed, e.g., via proper orthogonal decomposition applied to space--time snapshots). This latter issue precludes space--time projection methods from being used for, e.g., future state prediction. It is further worth noting that space--time projection comprises a fundamentally different approach than spatial projection. In ROMs leveraging space--time projection, the generalized coordinates (e.g., the reduced state) are a stationary vector, while the temporal dependence of the state is represented via a pre-defined basis. In ROMs leveraging spatial projection only, the generalized coordinates comprise a vector of \textit{time dependent functions}. Space--time projection executed at the time-continuous level yields an algebraic system of equations, while spatial projection executed at the time-continuous level yields a dynamical system.  
...
This work seeks to overcome the practical challenges associated with existing
residual minimization approaches for reduced-order models. To this end, we
propose the \methodNameLower\ (\methodAcronym) method for model reduction. The 
\methodAcronym\ approach sequentially minimizes the FOM ODE residual over a sequence of arbitrarily defined
time windows. The key novelties of the work are threefold.  First, the method
...
residual minimization problem, thus addressing a key deficiency of (space--time) LSPG. 
Time-continuous residual minimization also exposes
...
domain. This equips the method with additionally flexibility that enables it to
explore more fine-grained tradeoffs between computational cost and error. Finally, \methodAcronym\ is formulated for 
a general space--time trial space, thus allowing application to both classic spatial projection-only trial subspaces as well as  space--time projection trial subspaces; previous work on space--time residual minimization requires space-time trial subspaces obtained through space--time projection. We  
demonstrate that \methodAcronym\ is a generalization of existing
methods; particular instances of the formulation are equivalent to
...
The \methodAcronym\ approach is best viewed as a hybrid space--time method and displays 
...
formulation, however, was not fully developed and was eschewed for 
time-local residual minimization (i.e., LSPG). The present work thus
...
(1) in addition to trial subspaces obtained via space--time projection, we
consider trial subspaces obtained via spatial projection only (i.e., a classic
spatial basis is employed, opposed to a space--time basis)% -- in which case
%the generalized coordinates comprise a vector of functions
, and (2) we
...
consider trial subspaces obtained via both spatial projection only and space--time projection,
 and (3) residual minimization is performed over time
windows; Ref.~\cite{choi_stlspg} considers trial subspaces obtained via space--time projection and minimizes the
...
minimizes the residual of the FOM O$\Delta$E~\eqref{eq:lms} within the trial subspace over each time step. %We define the discrete residual for the implicit Euler method at the $n$th time-instance as,
%\begin{align*}
