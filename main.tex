...
\caption{Relationship diagram for the \methodAcronym\ approach for model
	reduction. \KTC{Also, can we make this more specific? In particular, can
	we write specific equations (e.g., ``$W = T$ in Eq.~(10)'') for the limiting
	cases?} \EP{I tried, it becomes very crowded. I think with the description of trial spaces its pretty good now}} 
\label{fig:flowchart} 
\end{centering} 
\end{figure}
...
\begin{enumerate} 
	\item \textit{Subspaces that reduce only the spatial dimension of the full-order
		model (\spatialAcronym)}. These trial subspaces are characterized by a \textit{spatial projection operator}, associate with a basis that represents the spatial dependence of the state, and are employed in classic model reduction approaches, e.g., Galerkin and LSPG. %These 
...
\section{Derivation of the Euler--Lagrange equations}\label{appendix:eulerlagrange}
	\KTC{In this section, $a$ and $b$ are sometimes used as time limits, while
	$0$ and $T$ are used elsewhere. Please go through carefully and make this
	consistent. I recommend using $a$ and $b$ because this is used within time
	windows.}
	\KTC{In this section, "extremum" is used a lot. You are just finding
	stationary points, which could be local minima, local maxima, or saddle
	points. Please avoid using "extemum" for this reason.} 
...
where $\integrand: \RR{M}  \times \RR{M}  \times[0,T] \rightarrow  \RRplus$
	...
	$\statez(0) = \statez_0$. \KTC{This previous sentence isn't correct. The
	functional defined here has two arguments; the extremum can't be a single
	argument, as it's a pair of arguments. You need to connect these by defining
	a new function that maps a time-dependent function to this functional
	evaluated at the time-dependent function and its derivative..} 
...
\KTC{Again, note that $\mathcal J$ has two arguments, so it can't havec a
single vector servec as an extremum. Fix needed}, then it must
...
either a maximum or a minimum. \KTC{See above. Strictly, these could be local
