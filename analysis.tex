...
The error in the solution computed by the \methodAcronymROMs\ method with
...
%$$ \int \norm{\resid{\state^*_R}}dt \le \int \norm{\resid{\tilde{\state}_f}}dt,$$
	\KTC{Not sure you can say the above. Does $\stateFOMProjSolArg{n}$ satisfy
	the same constraints as $\stateROMStarSolArg{n}$?}
...
(i.e., $\stateROMStarSolArgt{n}{t} = \basisspaceArg{n} \genstateROMStarSolArgt{n}{t} + \stateInterceptArg{n}$) and it is additionally noted that 
