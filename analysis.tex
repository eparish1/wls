...
In the limit of $\zeta \rightarrow 0^+$ \KTC{I added a superscript + here to
be consistent with the previous. Please check.} (and hence $\nslabs
\rightarrow \infty$) this is a (weak) statement of the Galerkin ROM. \KTC{I
...
\KTC{I'm not familiar with the above; can you double check and verify?}
Thus, we have
\begin{align*}
...
\KTC{In the rest of this section, you need to include the time argument for
$\stateROMSolArg{n}$ and $\stateROMStarSolArg{n}$ within the residual. Please
fix.}
\begin{multline*}
\intSlabArg{n} \norm{\stateROMSolArgt{n}{t} - \stateROMStarSolArgt{n}{t}} dt 
\le \\  \DeltaSlabArg{n} \sqrt{ \norm{\genstateROMSolArgt{n}{\timeStartArg{n}} - \genstateROMStarSolArgt{n}{\timeStartArg{n} }}^2 +   \norm{\basisspaceTArg{n} \resid( \basisspaceArg{n} \stateROMSolArg{n} + \stateInterceptArg{n} , {\timeStartArg{n}} ) - 
 \basisspaceTArg{n} \resid( \basisspaceArg{n} \stateROMStarSolArg{n} + \stateInterceptArg{n}, {\timeStartArg{n}}  )}^2  } .
\end{multline*}
As 
\begin{multline*}
...
\resid( \basisspace \stateROMStarSolArg{n} + \stateIntercept, {\timeStartArg{n}}  )}^2  } .
\end{equation*}
%\begin{equation*}
%\intSlabArg{n} \norm{\stateROMSolArgt{n}{t} - \stateROMStarSolArgt{n}{t}} dt 
...
\end{proof}
\begin{corollary}
	Under Assumption A2,  \KTC{Verify which specific assumptions are needed for
	this!}
...
Setting $n=1$ in~\eqref{eq:apriori_bound} with the time intervals
	$\timeStartArg{1}=0$, $\timeEndArg{1}=T$, employing Assumption A3 \KTC{Don't
	you only need Assumption A2?}, and noting that the initial conditions are known yields the desired result.
\end{proof}
