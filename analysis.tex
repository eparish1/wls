...
%\end{equation}
The \methodAcronym\ approach consists of solving the following sequence of minimization problems,
...
\subsection{A priori error bounds}
...
\begin{theorem}(\textit{A priori} error bounds for space--time trial space)\label{theorem:apriori_bound_st}
Error bounds for \methodAcronymROMs\ with a space--time trial space are:
\begin{equation}\label{eq:apriori_bound}
...
\end{align*}
Next, the results presented in Theorem~\ref{theorem:apriori_bound} hold for the space--time trial space through bound~\eqref{eq:boundtmp}. We now derive an upper bound for $\intSlabArg{n} \norm{ \stateROMSolArgt{n}{t} - \stateROMStarSolArgt{n}{t}}$ for the space--time case. By definition of the space--time trial space,
\begin{align*}
...
Theorems~\ref{theorem:apriori_bound} and~\ref{theorem:apriori_bound_st} provide \textit{a priori} bounds on the integrated normed error for the \methodAcronymROM\ for the spatial and space--time trial spaces. Several 
