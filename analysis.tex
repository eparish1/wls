...
%\end{equation}
Noting that $\variationArgn{n}$ is an arbitrary function, we take the limit $\zeta \rightarrow 0^+$ to collapse the window size and obtain the sequence of problems for $n=1,\ldots,\nslabs$,
\begin{equation*}
...
\end{equation*}
where it is noted that the first term in Eq.~\eqref{eq:g_equiv_1} vanishes as
 \begin{equation*}
...
In the following, the error over the $n$th window for $t \in [\timeStartArg{n},\timeEndArg{n}]$ is defined as
$$\errorArg{n}\vcentcolon \timeDummy \mapsto \stateFOMSolArg{}(\timeDummy)- \stateROMSolArg{n} (\timeDummy).$$
Additionally, we denote $\stateFOMProjSolArg{n}$, $n=1,\ldots,\nslabs$ to be the $\elltwo$-optimal solution over the $n$th window
...
Integrating over the $n$th window and using the definition of the error on the left hand side yields
...
\end{equation*}
Noting that, $\forall t \in [0,T]$, we have $\resid(\stateFOMSolArg{},t) = \bz$ and get the simplification 
