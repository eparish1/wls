...
discretization of the integral. We introduce $\ncollocSTArg{n} \ge \text{ceil}(\stdimArg{n}/ N )$ \EP{Just realized this actually shouldn't be N if we do hyper-reduction; should be something like Rank($\mathbf{W}^n)$ Agree?} collocation points over the $n$th window, 
