...
We now consider \spaceTimeAcronym\ trial subspaces. 
...
dimensional representation of the objective functional~\eqref{eq:obj_gen_slab}. We describe two techniques to do this: one that works with the FOM O$\Delta$E and one that works with the FOM ODE. We associate direct methods as those 
...
The direct solution technique seeks to minimize the fully discrete objective function associated with the O$\Delta$E. For brevity, we focus on linear multistep methods and leverage the time grids introduced in Section~\ref{sec:direct}. For notational simplicity, we define an index mapping function that is 
