...
\item replace $J_{D-ST}^n$ in (38) with $J_D^n$. You can do this because of Eq 
	(33) \EP{Yes, done}
\item remove definition of $J_{D-ST}$. \EP{Yes, done}
 \end{enumerate}
}
...
\EP{I noticed you got rid of the assumption about the size of the generalized coordinates. Why don't we need to worry about this?}
...
For the limiting case where one window comprises entire domain (i.e.,
	...
	space is set to be $\stspaceSTArg{1} = \stspaceST$, the weighting matrix $\lspgWeightingST = \text{diag}(\stweightingMatOneArg{1})$
	\KTC{Don't we just need simply
	$W_{ST}=\text{diag}(W^1)$ (with $W^1$ the half-factor of $A^1$)?. I think this
	would be more general.}\EP{Yes, this is better. Is diag the right command though? It should be $W_{ST} = [W \; W \; W \; W]$}, and $\nstepsArg{1} = N_t$ time instances are employed that
