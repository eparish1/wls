...
where $\quadWeightsLMSScalarArg{n}{i} \in \RR{}$ are quadrature weights. \methodAcronym\ solves the minimization problem over each time window, 
\begin{align*}
...
\begin{algorithm}
\caption{\spatialAcronym\ trial space: algorithm for the direct solution technique with the Gauss--Newton method and a linear multistep method over the $n$th window}
\label{alg:colloc_gn}
...
\begin{algorithm} \caption{\spatialAcronym\ trial space: algorithm for the FBSM over the $n$th window.} \label{alg:st_iter} 
\SetKwInOut{Input}{Input}\SetKwInOut{Output}{Output}
