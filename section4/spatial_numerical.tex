...
\KTC{Passive voice:} \EP{I think this reads clearly} The minimization problem is then reformulated as a nonlinear programming, or
...
\KTC{you can't devide (i.e., decompose) a window into instances. you can divide a window into
...
\includegraphics[trim={0.0cm 5cm 0cm 3cm},clip,width=1.0\textwidth]{figs/time_grid_timesteps.pdf} 
	\caption{Partitioning of time windows into time-step instances. \KTC{Don't call them `time-step instances'.
...
\end{bmatrix},
\end{equation*}
where $\genstatecollocMatySlabArg{} \defeq \unroll (\genstateyDiscreteArg{0},\ldots,\genstateyDiscreteArg{\nstepsArg{n}} ).$
...
\Input{tolerance, $\text{tol}$; initial guess, $\genstateGuessDiscreteArg{n,1}{0},\ldots,\genstateGuessDiscreteArg{n,\nstepsArg{n}}{0}$; initial condition $\genstateDiscreteArg{n,0}$}
...
$\genstatecollocMatSlabArg{n}_k \leftarrow \unroll(\genstateDiscreteArg{n,0},\genstateGuessDiscreteArg{n,1}{0},\ldots,\genstateGuessDiscreteArg{n,\nstepsArg{n}}{0})$ \Comment{Assemble generalized coordinates over window} \\
