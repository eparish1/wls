...
\EP{Shouldn't this be subset? We can't recover the continuous time interval}
\KTC{Yes},
$n=1,\ldots,\nslabs$ that
satisfy 
...
\end{cases}$$
\KTC{Note: I added parenthesis here because a function can't really map to a
pair of indices unless they are defined as a collection or tuple. Should be
fine.}
...
\end{align*}
where
\KTC{This next equation: I don't think you should project the previous
solutions.}
\begin{equation*}
\begin{split}
\objectiveArgLMS{n}_{\text{D-S}} &\vcentcolon  (\stateyDiscreteArgnt{1},\ldots,\stateyDiscreteArgnt{\nstepsArg{n}}) \mapsto \objectiveArgLMS{n}(\stateyDiscreteArgnt{1},\ldots,\stateyDiscreteArgnt{\nstepsArg{n}}; \projectorSpatialArg{n}(\approxstateDiscreteArg{\indexMapper(n,0)}),\ldots,\projectorSpatialArg{n}(\approxstateDiscreteArg{\indexMapper(n,-\lmsWidthArg{n}{1}+1)})), \\
...
A variety of algorithms exist for solving nonlinear least-squares problems,
	...
	\KTC{Removed `trust-region approaches', because Levenberg--Marquardt is a
	trust-region approach. Also added en-dashed.} The numerical experiments
...
\KTC{This is a fantastic insight and a really great contribution!}
