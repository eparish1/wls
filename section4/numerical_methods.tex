...
numerically discretizing the system \KTC{What does it mean to discretize the
system?} and objective functional and ``transcribing"
the infinite-dimensional problem into a finite-dimensional 
problem. Direct approaches thus
``discretize then optimize''. \KTC{Note: only use hyphens when
discretize-then-optimize is used as an adjective.}
...
first-order optimality conditions). \KTC{Note: they are not first-order
optimality conditions of the objective functional only; they are first-order
optimality conditions of the optimization problem} Thus, indirect methods
